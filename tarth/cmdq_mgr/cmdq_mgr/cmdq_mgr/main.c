...
void main() 
{
	...
	struct node *activenode[8][10];
	int data = 0xffff1234;
	int key = 1234;

	struct qentry_struct *newentry[8][10];
	...
	for (i = 0; i < QP_MAX; i++)
	{
		for (j = 0; j < MAXTEST; j++)
		{
			newentry[i][j] = malloc(sizeof(struct qentry_struct));

			//struct node *nnode = (struct node*)malloc(sizeof(struct node));
			//newnode[i][j] = nnode;

			newentry[i][j] = cmdq_mgr_node_get();

			if (newentry[i][j] == NULL)
			{
...
/*
	...
	for (i = 0; i < QP_MAX; i++)
	{
		for (j = 0; j < MAXTEST; j++)
		{
			if (cmdq_mgr_tail_insert(&newentry[i][j], i) != 0)
			{
	...
	for (i = 0; i < QP_MAX; i++)
	{
		for (j = 0; j < MAXTEST; j++)
		{
			cmdq_mgr_send_to_active(&newentry[i][j], i);
		}
	}
	...
	for (i = 0; i < QP_MAX; i++)
	{
		for (j = 0; j < MAXTEST; j++)
		{
			cmdq_mgr_node_return(&newentry[i][j], i);
		}
	}
}