...
Run the Receiver application and open menu item Setup/Receiver Card (Edit).
...
Further settings are described bellow. However, be careful when changing them.
We suggest to use default values as shown in the table.

Setting		  default	  comment
-----------------------------------------------------
[CardState]
Flags		= 0x72		 * do not change
ACKtimeout	= 10		 * [ms], card waits this long if less than 31 packets available when driver requests data
Speed 		= 10000		 * [kHz], frequency of the card, available values 10000 - 16000


[DriverState]
DriverMemory= 200		 * queue size for the DVB driver (# of packets),
Flags		= 0			 * Driver debug options: 1-dump packet contents, 2-dump packet id's

"CardState.Flags" value is hexadecimal number consisting of following bits:
	mask 0x0E: input packet (received from demultiplexor IRD device) characteristics:
		flags			meaning
		---------------------------------------
		0x00		... MUX184
		0x02, 0x0A	... TS4+MUX184
		0x04, 0x0C	... PESH6+MUX178
		0x06, 0x0E	... TS4+PESH6+MUX178
		0x08		... MUX178
